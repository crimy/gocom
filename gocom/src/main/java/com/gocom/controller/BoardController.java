...
EmojiService emoService;
	...
	public ModelAndView upload(MultipartHttpServletRequest request, HttpServletResponse response ) throws Exception {
		...
		for( MultipartFile file : files ) {
			...
			if( fsn.endsWith("_") ) {
				AlertUtil.alertAndBackPage(response, "파일을 업로드해야합니다");
				mnv.setViewName("redirect:/board/write");
				return mnv;
			}
			fsnlist.add( fsn );
		}
	...
	@RequestMapping(value="reactEmoji", method = RequestMethod.POST)
	public ModelAndView reactemoji(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		if( name == "" || name == null ) {
			...
			if( result == 1 ) {		
				AlertUtil.alertAndMovePage(response, "등록됐습니다","/gocom/board/view?no="+con_no);
			} else {
				AlertUtil.alertAndBackPage(response, "다시 시도해주세요");
			}
	...
	public ModelAndView view(HttpServletRequest request) throws Exception {
		...
		ImageDTO first = i_list.get(0);
		if( i_list.size() > 1 ) {
			List<ImageDTO> i_list2 = i_list.subList(1, i_list.size() );
			mnv.addObject("image", i_list2);
		}
		else {
			mnv.addObject("image", i_list);
		}
		
		mnv.addObject("image_first", first);
		mnv.addObject("i_count", i_list.size() );
		...
		try {
			if( userid != null ) {			
				int result1 = boardService.recommend(con_no,userid);					
					AlertUtil.alertAndMovePage(response, "추천했습니다","/gocom/board/view?no="+con_no);
	...
	public ModelAndView warn(HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		try {
			if( userid != null ) {			
				...
				AlertUtil.alertAndBackPage(response, "반대했습니다");
